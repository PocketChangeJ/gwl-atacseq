...
merged_peaks = read.delim(file_mergedpeaks, header = F, stringsAsFactors = F)
samplelist = read.delim(file_sampleinfo, header = F, stringsAsFactors = F)

# Setup header
row.names(merged_peaks) = merged_peaks[,8]
merged_peaks = merged_peaks[,-8]
names(merged_peaks) = c("chr", "start", "end", "hits", "mean_score", "nearest_gene", "distance_TSS")
...
for(i in 1:nrow(samplelist))
{
	...
	input_file = read.delim(row[,2], header = F, stringsAsFactors = F)
